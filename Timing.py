...
def buscar_citas_dia_siguiente():
 ...
 #
        ...
        for i in range(0, len(pekeslist)):
            minDate +=2
            tfn = pekeslist[i][2]
            progenitor1 =pekeslist[i][1]
            peke = pekeslist[i][0]
            hora = citasList[i][3]
           # print(f"{minDate} es el Mindate antes del primer mensaje")
            pywhatkit.sendwhatmsg_instantly(
            phone_no="+34{}".format(tfn), 
            message="Hola {} mañana {} tiene una cita a las {}. *Este es un mennsaje automático y no hace falta responder a este mensaje*".format(progenitor1, peke, hora),
          #  time_hour=hourDate,
          #  time_min=minDate, wait_time= 15, 
            tab_close=True, close_time= 15)
            print('Sending msg1')
            minDate += 2
            tfn2 = pekeslist[i][4]
            progenitor2 =pekeslist[i][3]
          #  print(f"{minDate} es el Mindate antes del Segundo mensaje")
            pywhatkit.sendwhatmsg_instantly(
            phone_no="+34{}".format(tfn2), 
            message="Hola {} mañana {} tiene una cita a las {}. Si no hay ningún inconveniente, *No hace falta responder a este mensaje*".format(progenitor2, peke, hora),
            #time_hour=hourDate,
            #time_min=minDate, wait_time= 15,
            tab_close=True, close_time= 15)
            print('Sending msg2')
